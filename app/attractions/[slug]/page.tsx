    ...
    <section className="px-5 py-16">
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-8">
          ...
          <div className="mt-6 flex flex-wrap gap-2 text-sm"><span className="rounded-full bg-white/10 px-4 py-2">{event.venueName}</span><span className="rounded-full bg-white/10 px-4 py-2">{formatPrice(event.priceMin)}</span>{event.runtimeMinutes ? <span className="rounded-full bg-white/10 px-4 py-2">{event.runtimeMinutes} minutes</span> : null}</div>
