      ...
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-8">
          ...
          <div className="mt-6 flex flex-wrap gap-2 text-sm">
            <span className="rounded-full bg-white/10 px-4 py-2">{event.venueName}</span>
            <span className="rounded-full bg-white/10 px-4 py-2">{formatPrice(event.priceMin)}</span>
            {event.runtimeMinutes ? <span className="rounded-full bg-white/10 px-4 py-2">{event.runtimeMinutes} minutes</span> : null}
          </div>
          <div className="mt-8 grid gap-5 md:grid-cols-2">
            <div><h2 className="font-black text-white">Best for</h2><ul className="mt-3 space-y-2 text-white/70">{event.bestFor.map((item) => <li key={item}>• {item}</li>)}</ul></div>
            <div><h2 className="font-black text-white">Skip if</h2><ul className="mt-3 space-y-2 text-white/70">{event.skipIf.map((item) => <li key={item}>• {item}</li>)}</ul></div>
          </div>
