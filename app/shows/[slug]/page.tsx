...
export function generateStaticParams() {
  return seedEvents.filter((event) => event.category === "shows").map((event) => ({ slug: event.slug }));
}

export default async function ShowDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const event = getEventBySlug(slug);
  if (!event || event.category !== "shows") notFound();
      ...
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-8">
          ...
          <div className="mt-8 grid gap-5 md:grid-cols-2">
            ...
            <div>
              <h2 className="font-black text-white">Skip if</h2>
