    ...
    <article className="group overflow-hidden rounded-lg border border-white/10 bg-white/[0.06] shadow-2xl shadow-black/20 transition hover:-translate-y-1 hover:border-amber-100/25 hover:bg-white/[0.09]">
      ...
      <div className="space-y-4 p-5">
        ...
        <div className="grid gap-3 pt-2 sm:grid-cols-[auto_1fr]">
          <Link href={eventPath} className="inline-flex items-center justify-center gap-2 rounded-full border border-white/15 px-4 py-2 text-sm font-bold text-white transition hover:bg-white/10">
            Details <ArrowRight className="h-3.5 w-3.5" />
          </Link>
          {hasTicketUrl ? (
            <a href={event.affiliateUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-black text-black transition hover:bg-amber-100">
              <Ticket className="h-3.5 w-3.5" /> Check Tickets
            </a>
          ) : (
            <Link href={eventPath} className="inline-flex items-center justify-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-black text-black transition hover:bg-amber-100">
