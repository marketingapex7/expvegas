          ...
          {directoryNav.map((item) => {
            ...
            return (
              <div
                ...
                onFocusCapture={() => setDesktopMenu(item.label)}
                onBlurCapture={(event) => {
                  if (!event.currentTarget.contains(event.relatedTarget as Node)) setDesktopMenu("");
                }}
              >
                ...
                {open ? (
                  <div className="absolute left-1/2 top-full w-[34rem] -translate-x-1/2 pt-3">
                    <div className="rounded-lg border border-white/10 bg-[#100e15] p-5 shadow-2xl shadow-black/60">
