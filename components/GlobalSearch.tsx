...
export function GlobalSearch() {
  ...
  return (
    <>
      ...
      {open ? (
        <div className="fixed inset-0 z-[90] bg-black/75 px-4 pt-[12vh] backdrop-blur-sm" onMouseDown={(event) => { if (event.currentTarget === event.target) close(); }}>
          <section role="dialog" aria-modal="true" aria-label="Search ExperienceVegas" className="mx-auto max-h-[72vh] max-w-2xl overflow-hidden rounded-lg border border-white/15 bg-[#100e15] shadow-2xl">
            <div className="flex items-center gap-3 border-b border-white/10 p-3">
              ...
              <button type="button" onClick={close} aria-label="Close search" className="inline-flex h-10 w-10 items-center justify-center rounded-lg text-white/60 hover:bg-white/10 hover:text-white"><X className="h-5 w-5" /></button>
            </div>
            <div className="max-h-[58vh] overflow-y-auto p-2">
              {results.length ? results.map((item) => {
                const Icon = categoryIcon[item.category];
                return (
                  <Link key={`${item.category}-${item.id}`} href={item.href} onClick={close} className="flex items-start gap-3 rounded-lg px-3 py-3 text-left transition hover:bg-white/[0.07]">
