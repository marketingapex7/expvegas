    ...
    <div className="min-w-0">
      ...
      <article className="grid gap-3 rounded-lg border border-white/10 bg-black/25 p-3 sm:grid-cols-[5.5rem_minmax(0,1fr)] sm:p-4">
        ...
        <div className="min-w-0">
          ...
          {block.description ? <p className="mt-2 hidden text-sm leading-6 text-white/62 sm:block">{block.description}</p> : null}
...
export function HomeLivePlan({
  ...
  return (
    <section id="trip-builder" className="relative overflow-hidden px-4 pb-10 pt-8 sm:px-5 sm:pb-14 sm:pt-12">
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h1 className="mt-3 text-4xl font-black leading-[1.02] text-white sm:text-5xl lg:text-6xl">See how your Vegas weekend fits together.</h1>
          <p className="mx-auto mt-4 max-w-2xl text-base leading-7 text-white/70 sm:text-lg">Start with a useful plan. Change the dates, group size, or spend level, then refine the parts that matter to you.</p>
        </div>

        <div className="mt-7 rounded-lg border border-white/12 bg-white/[0.07] p-3 shadow-2xl shadow-black/30 sm:p-4">
          <div className="grid gap-2 md:grid-cols-[minmax(17rem,1.6fr)_minmax(10rem,0.7fr)_minmax(10rem,0.8fr)_auto]">
            <div className="grid grid-cols-2 gap-2 rounded-lg border border-white/10 bg-black/25 p-2">
            ...
            <Link href={plannerHref} onClick={handoffToPlanner} className="inline-flex min-h-14 items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-amber-300 to-fuchsia-300 px-5 py-3 text-sm font-black text-zinc-950 transition hover:brightness-110">
        ...
        {firstDay ? (
          <div className={`mt-5 rounded-lg border border-white/12 bg-white/[0.055] p-4 transition sm:p-6 ${updating ? "opacity-65" : ""}`} data-testid="homepage-live-plan">
            <div className="flex flex-col gap-3 border-b border-white/10 pb-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.18em] text-amber-100">Day 1 · {firstDay.label}</p>
                <h2 className="mt-2 text-2xl font-black text-white sm:text-3xl">{firstDay.theme}</h2>
              </div>
              <p className="inline-flex items-center gap-2 text-xs font-bold text-white/48"><Sparkles className="h-4 w-4 text-fuchsia-300" /> Timed by the real planner engine</p>
            </div>
