...
function scheduledFallbackEvents(startDate?: string, endDate?: string) {
...
export async function generatePlannerResponse(input: PlannerInput): Promise<PlannerResponse> {
  ...
  try {
    liveEvents = await searchTicketmasterEvents({ startDate, endDate, category, size: 40 });
  } catch {
    liveEvents = [];
  }

  const fallbackSchedule = liveEvents.length > 0 ? [] : scheduledFallbackEvents(startDate, endDate);
  ...
  return {
    ...
    itineraryDays,
    tripSummary: buildTripSummary(input, itineraryDays, best),
  };
}
